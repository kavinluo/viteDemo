<template>
  <!--:style="{width: w + 'px'}"-->
  <div v-show="isVerificationShow" id="slide-verify" onselectstart="return false;">
    <!-- 图片加载遮蔽罩 -->
    <div :class="{'slider-verify-loading': loadBlock}"></div>
    <canvas :width="w" :height="h" ref="canvas" ></canvas>
    <div>
      <span class="slide-verify-refresh-icon" @click="refresh"><i style="font-size: 20px" class="el-icon-refresh-right"></i></span>
      <span class="slide-verify-close-icon" @click="closeVerificationBox"><i style="font-size: 20px" class="el-icon-close"></i></span>
      <!--<span class="close-btn iconfont iconclose-bold" @click="closeVerificationBox"></span>-->
    </div>
...
import img9 from '@/assets/verificationImage/9j.png'
  // let img1 = require('../../assets/verificationImage/1j.jpg')
  // let img3 = require('../../assets/verificationImage/3j.jpeg')
  // let img4 = require('../../assets/verificationImage/4j.png')
  // let img5 = require('../../assets/verificationImage/5j.png')
  // let img6 = require('../../assets/verificationImage/6j.png')
  // let img7 = require('../../assets/verificationImage/7j.jpg')
  // let img8 = require('../../assets/verificationImage/8j.png')
  // let img9 = require('../../assets/verificationImage/9j.png')
  const PI = Math.PI;
  ...
  defineEmits(['update:isVerificationModel'])
  // export default {
    const props = defineProps({
      ...
      imgs: {
        type: Array,
        default: () => [img1,img3,img4,img5,img6,img7,img8,img9],
      },
      verificationShow: {
        type: Boolean,
        default: false
      ...
      const  block_x = ref(undefined)// container random position
      const  block_y = ref(undefined)
      const  L = ref(props.l + props.r * 2 + 3) // block real lenght
      ...
      const success = ref(false) // Bug Fixes 修复了验证成功后还能滑动
      const loadBlock = ref(true) // Features 图片加载提示，防止图片没加载完就开始验证
...
<style scoped>
  ...
  .slide-verify-slider-mask-item-icon {
    ...
    width: 14px;
    height: 12px;
    background: url("../../assets/verificationImage/icon_light.png") 0 -26px;
    background-size: 34px 471px
  }
